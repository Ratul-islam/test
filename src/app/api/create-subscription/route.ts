...
// For error handling
type StripeErrorResponse = Error & {
  raw?: unknown;
};
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Create Stripe checkout session - REMOVED receipt_email
    ...
    const stripeError = error as StripeErrorResponse;
    console.error('[ERROR] Create subscription failed:', {
      message: stripeError.message,
      stack: stripeError.stack,
      ...(stripeError.raw ? { stripe_error: stripeError.raw } : {})
    });
